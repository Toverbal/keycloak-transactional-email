...
/** Integration tests for the {@code /realms/{realm}/ext-email-template} REST resource. */
class TransactionalEmailResourceTest extends AbstractTransactionalEmailTest {
  ...
  void getConfig_returnsEmptyConfigWhenNothingSet() {
    TransactionalEmailConfig config =
        given()
            .auth()
            .oauth2(adminToken())
            .when()
  ...
  void putConfig_storesProviderAndTemplateMappings() {
    TransactionalEmailConfig toSave =
        new TransactionalEmailConfig(
            "sendgrid",
    ...
    TransactionalEmailConfig loaded =
        given()
            .auth()
            .oauth2(adminToken())
            .when()
  ...
  void putConfig_secretMaskDoesNotOverwriteStoredKey() {
    ...
    TransactionalEmailConfig initial =
        new TransactionalEmailConfig(
    ...
    TransactionalEmailConfig roundTrip =
        new TransactionalEmailConfig(
    ...
    // The stored key should still be masked (meaning the real key was preserved, not overwritten)
    TransactionalEmailConfig loaded =
        given()
            .auth()
            .oauth2(adminToken())
  ...
  void putConfig_clearingProviderDisablesRouting() {
    TransactionalEmailConfig initial =
        new TransactionalEmailConfig("sendgrid", Map.of(), Map.of());
    given()
        .auth()
        .oauth2(adminToken())
        .contentType("application/json")
    ...
    TransactionalEmailConfig cleared =
        new TransactionalEmailConfig("", Map.of(), Map.of());
    given()
        .auth()
        .oauth2(adminToken())
        .contentType("application/json")
