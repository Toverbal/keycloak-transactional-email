...
public abstract class AbstractAdminResource {
  ...
  private void setupEvents() {
    adminEvent =
        new AdminEventBuilder(realm, auth, session, session.getContext().getConnection())
            .realm(realm);
    event = new EventBuilder(realm, session, connection).realm(realm);
  }
  ...
  private AdminAuth authenticateRealmAdminRequest(HttpHeaders headers) {
    ...
    RealmModel tokenRealm = realmManager.getRealmByName(realmName);
    if (tokenRealm == null) {
    ...
    ClientModel client = tokenRealm.getClientByClientId(token.getIssuedFor());
    if (client == null) {
      throw new NotFoundException("Could not find client for authorization");
    }

    return new AdminAuth(tokenRealm, authResult.getToken(), authResult.getUser(), client);
  }
}
